...
PLAYER = {
    ...
    "down": [images.spacesuit_front, images.spacesuit_front_1,
            images.spacesuit_front_2, images.spacesuit_front_3, images.spacesuit_front_4]
}
...
def draw_image(image, y, x):
    screen.blit(
        image, (top_left_x + (x * TILE_SIZE), top_left_y + (y * TILE_SIZE) - image.get_height())
    )
        ...
        for x in range(room_width):
            ...
            if item_here not in items_player_may_stand_on + [255]:
                ...
                if objects[item_here][1] is not None: # If object has a shadow
                    ...
                    #if shadow might need horizintal tiling
...
assert len(props) == 37, "Expected 37 props items"
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]: # places you can drop things
        ...
        time.sleep(0.5)
    else: # This happens only if there is already a prop here
...
def use_object():
    global room_map, item_carrying, air, selected_item, energy
    ...
    if suit_stitched and air_fixed: # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            sounds.say_airlock_open.play()
            show_text("The computer tells you the sirlock is now open.", 1)
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1 and item_player_is_on == ingredient2) \
            ...
            if item_player_is_on in props.key():
...
def game_completion_sequence():
    ...
    if launch_frame < 9:
        ...
        draw_shadow(images.rescue_ship_shadow, * + launch_frame, 6) #TODO: this may be a bug in the future
...
def door_in_room_26():
    ...
    frames = [images.door, images.door1, images.door2, images.door3,
            images.door4, images.floor]

    shadow_frames = [images.door_shadow, images.door1_shadow, images.door2_shadow, images.door3_shadow,
                    images.door4_shadow, None]

    if current_room != 26:
        clock.unscheduler(door_in_room_26)
        return
    ...
    if ((player_y == 8 and player_x ==2) or props[63] == [26, 8, 2]) \
        ...
        if airlock_door_frame == 5:
            props[21][0] = 0 # Remove door fram map when fully open
            room_map[0][1] = 0
            room_map[0][2] = 0
            room_map[0][3] = 0
