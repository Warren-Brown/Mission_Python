...
#TODO:
#input the correct values for the game map below
...
#using room data, scenery data and prop data.
    ...
    for y in range(room_height - 2):
        room_map.appent([side_edge] + [floor_type]*(room_width - 2) + [side_edge])
    ...
    if current_room <= MAP_SIZE - MAP_WIDTH: #If room is not on bottow row
        ...
        if room_below[3]:
            rom_map[room_height - 1][middle_column] = floor_type
            rom_map[room_height - 1][middle_column + 1] = floor_type
            rom_map[room_height - 1][middle_column - 1] = floor_type
